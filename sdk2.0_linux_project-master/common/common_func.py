...
def get_screen_shot(appname):
    ...
    # command1 = 'gnome-screenshot'
    command = 'gnome-screenshot -f' + ' ./screenshot/' + \
              appname + time.strftime('%Y-%m-%d-%H-%M-%S') + '.png'
    os.system(command)
...
'''def create_bat_for_launch(app_name):
    ...
    mkdir('./tmp')
    with open('./tmp/' + 'start_app.bat' , 'w') as f:
...
'''def launch_app():
    ...
    if os.path.exists(base + '/tmp/start_app.bat'):
        command = base + '/tmp/start_app.bat'
        subprocess.Popen(command, stdout=subprocess.PIPE, shell=True)
    else:
        logging.info('启动文件不存在')
        logging.info(base + '/tmp/start_app.bat')'''
...
def launch_app(app):
    command = './' + app
    launchapp = subprocess.Popen(command, stdout=subprocess.PIPE, shell=True, \
                                 cwd = '/home/test/Programe/AstraSDK-v2.0.13-5a10c0cc62-20181122T123809Z-Linux/bin', \
                                 executable = '/bin/bash')
    time.sleep(7)
    app_pid = launchapp.pid
    close_app(app_pid + 1)
...
def get_app_output(appname):
    ...
    # base = os.path.dirname(os.path.dirname(__file__))
    base = get_config_data()['tool_base']
    command = data + appname + ' > ' + base + './tmp/content.txt'
    p = subprocess.Popen(command, stdout=subprocess.PIPE, shell=True)
    sleep(15)
    os.system('kill -9 ') + p.pid
    # os.system('TASKKILL /PID %s /T /F' % p.pid)

def content_verify(app_name, worksheet, index):
    sleep(5)
    file = open('./tmp/content.txt', 'r')
    out_put = file.read()
